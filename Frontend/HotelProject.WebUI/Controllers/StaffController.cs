...
namespace HotelProject.WebUI.Controllers
{
    public class StaffController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("http://localhost:5000/api/Staff");// veri
             // listelemede getasync kullanılır. 
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<List<StaffViwModel>>(jsonData);
                // dESERALİZE veri listeleme de kulalnılır list alması için uide model açılır.
